...
<form method="post" action="resetcode.jsp" name="ResetPasswordForm">
...
<p>New Password</p>
<input type="password" required="" title="Password must contain atleast 6 characters including uppercase/lowercase letters,numbers" required pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}" name="npassword" placeholder="Enter new password"/></br>
...
<input type="submit" name="changepassword" value="Reset Password" onclick="confirmPass()"/>
</form>
</div>
    <script>
        function confirmPass()
        {
            var confirm=document.ResetPasswordForm.cpassword.value;
            var password=document.ResetPasswordForm.npassword.value;
            if(confirm!=password)
            {
